...
def alteracaofor():
    ...
    if len(nomemem)==0 or len(nomemem) >50:
        ...
        tela.nome.focus()
        return
    if len(enderecomem)==0 or len(enderecomem)>50: 
        messagebox1("Informação: Endereço esta vazio ou é maior que 50",manutencao)
        tela.endereco.focus()
        return
    if len(telefonemem)==0 or len(telefonemem)>11:
        messagebox1("Informação: digite o Nome esta vazio ou é maior que 11",manutencao)
        tela.telefone.focus()
        return    
    if len(tipomem)!=1 or tipomem not in ("F","J", "f", "j"):
        messagebox1("Informação: tipo  tamanho 1 e pessoa (F)isica ou (J)urica",manutencao)
        tela.tipo.focus()
        return
    if len(cepmem) != 8:
        ...
        tela.cep.focus()
        return
    if  tipomem in ("F","f") and len(cpfmem) !=11:
         messagebox1("Cpf tem que ser tamanho 11 ",manutencao)
         tela.cpf.focus()
    ...
    if tipomem in ("J","j") and len(cnpjmem) !=14:
               messagebox1("Cnpj tem que ser tamanho 14 ",manutencao)
               tela.cnpj.focus()
               if  len(cpfmem) !=0:
                 messagebox1("Você digitou Pessoa Juridica então não tem CPF ",manutencao)
...
def fornecedor_menu(janela1):
 ...
 janela3.title("Menu Manutenção - Fornecedor Consultas Relatorios  F1 - PARA SAIR")
